...
async function seed() {
  ...
  const user = await User.findOneAndUpdate(
    { clerkId: CLERK_ID },
    { clerkId: CLERK_ID, email: "test@vedaai.com", firstName: "Demo", lastName: "Teacher" },
    { upsert: true, new: true }
  );
  console.log("User:", user._id);

  // Clean up existing seed data for this user
  const existing = await Assignment.find({ teacher: user._id });
  for (const a of existing) {
    await Section.deleteMany({ assignment: a._id });
    await Question.deleteMany({ assignment: a._id });
  }
  ...
  const s4 = await Section.create({ assignment: a2._id, name: "Section A – MCQ", order: 0, instructions: "Each question carries 1 mark." });
  await Question.insertMany([
  ...
  const s5 = await Section.create({ assignment: a2._id, name: "Section B – Fill in the Blanks", order: 1, instructions: "Fill in the blanks. Each carries 1 mark." });
  await Question.insertMany([
  ...
  const s6 = await Section.create({ assignment: a2._id, name: "Section C – Short Answer", order: 2, instructions: "Answer in 2–3 sentences. Each carries 3 marks." });
  await Question.insertMany([
