...
interface GeneratedOption { label: string; text: string; }
...
interface GeneratedPaper { sections: GeneratedSection[]; }
...
function buildPrompt(assignment: {
  ...
  difficulty: string;
  questionTypeDetails: { type: string; count: number; marks: number }[];
...
function normaliseType(raw: string): "MCQ" | "Short Answer" | "Long Answer" | "Fill in the Blank" | "True/False" {
  const r = raw.toLowerCase();
  if (r.includes("multiple") || r.includes("mcq")) return "MCQ";
  if (r.includes("long"))   return "Long Answer";
  if (r.includes("short"))  return "Short Answer";
  if (r.includes("fill"))   return "Fill in the Blank";
  if (r.includes("true"))   return "True/False";
  return "Short Answer";
}

function normaliseDifficulty(raw: string): "Easy" | "Moderate" | "Hard" {
  const r = raw.toLowerCase();
...
export async function generateAssignment(assignmentId: string): Promise<void> {
  ...
  try {
    ...
    const prompt = buildPrompt({
      ...
      questionTypeDetails: assignment.questionTypeDetails as { type: string; count: number; marks: number }[],
    ...
    const paper: GeneratedPaper = JSON.parse(jsonText);
